...
import { AttachEmail, Replay, Save }                 from "@mui/icons-material";
...
function MailReferences() {
    ...
    const saveReference = ()=>{
        ...
        if(reference !== null){
            ...
            putUrl(formatURL(location.state._links.self.href) + "/referencedMails", aux.map(row => row._links.self.href).join("\n"), "text/uri-list").then(() =>{
                getUrl(formatURL(location.state._links.self.href) + "/referencedMails?projection=documentList").then((response) => {
    ...
    const attachedClickHandler = (event : React.MouseEvent<HTMLElement>, modelId: any) => {
        event.preventDefault();
        navigate("/mail/references/" + params.action, { state: modelId } );
    }
    ...
    const Actions = (entity : any) =>{
        return(
            <>
                ...
                <Tooltip title="References" arrow>
    ...
    const toParent = () => {
        navigate("/mail/" + params.action , {state : location.state._links.self.href})
    }
                    ...
                    <Table stickyHeader onMouseLeave={event => rowHoverHandler(event, -1)}>
                        <TableHead onMouseEnter={event => rowHoverHandler(event, -1)}>
                            <TableRow sx={{ cursor:"pointer" }}>
                                ...
                                <TableCell align="center" style={{ backgroundColor: '#555', color: 'white', width:'100px'}} key={Lists.get("mail").length}>&nbsp;</TableCell>
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            {rows && rows
