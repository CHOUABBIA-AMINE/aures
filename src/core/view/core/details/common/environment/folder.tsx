...
import { Folder } 				from "../../../../../model/common/environment/folder";

const FolderDetails = (props : any) => {
	...
	const [ archiveBoxs, setArchiveBoxs ]	= useState<ArchiveBox[]>([]);
	const [ archiveBox, setArchiveBox ]		= useState<string>("");
	const [ _archiveBox, set_ArchiveBox ]	= useState<string>("");
		...
		if(location !== null && location.state !== null && location.state.modelId !== null ){
			getUrl(formatURL(location.state.modelId)).then((response : any) => {
				setFolder({
					...
					_links          : {
						folder     		:{
							href            : response.data._links.folder.href
						},
						self            :{
							href            : response.data._links.self.href
						},
						archiveBox  	:{
							href            : response.data._links.archiveBox.href
						}
					}
				})
				getUrl(formatURL(response.data._links.archiveBox.href)).then((archiveBox) => {
					set_ArchiveBox(archiveBox.data._links.self.href);
					getUrl(formatURL(archiveBox.data._links.shelf.href)).then((shelf) => {
					...
					getUrl(formatURL(archiveBox.data._links.shelfFloor.href)).then((shelfFloor) => {
	...
	const patchData = ()=>{
		if(location !== null && location.state !== null && location.state.modelId !== null){
			patchUrl(formatURL(location.state.modelId), JSON.stringify({
				code   			: folder.code,
				designationAr   : folder.designationAr,
				designationEn   : folder.designationEn,
				designationFr   : folder.designationFr,
				archiveBox		: archiveBox
			})).then((response) => {
				enqueueSnackbar('Entity updated successfully!', {variant: 'success'});
			})
		}else{
			postBasedUrl("folder", JSON.stringify({
				code   			: folder.code,
				designationAr   : folder.designationAr,
				designationEn   : folder.designationEn,
				designationFr   : folder.designationFr,
	...
	useEffect(() => {
		if(shelf !== ""){
			getUrl(formatURL(shelf) + "/archiveBoxs").then((archiveBoxs) => {
				setArchiveBoxs(archiveBoxs.data._embedded.archiveBox);
			});
		}
    },[shelf])
	...
	useEffect(() => {
		...
		if(archiveBoxs.length !== 0 && _archiveBox !== ""){
			console.log(_archiveBox)
			setArchiveBox(_archiveBox);
			set_ArchiveBox("");
		}
    },[archiveBoxs])/**/
		...
		<Container maxWidth="lg">
			<Paper variant="outlined" sx={{ marginTop: "60px", padding:'30px' }}>
				...
				<Grid container spacing={1}>
					...
					<Grid item xs={3} sm={3}>
						<FormControl fullWidth size="small" >
							<InputLabel id="archiveBoxLabel">Archive Box</InputLabel>
							<Select
								required
								fullWidth
								size="small"
								labelId="archiveBoxLabel"
								id="archiveBox"
								variant="outlined"
								value={archiveBox}
								label="archiveBox"
							...
							>
								{
									archiveBoxs.length > 0 && archiveBoxs.map(archiveBox => {
										return(
											<MenuItem key={archiveBox._links.self.href} value={archiveBox._links.self.href}>{archiveBox.code}</MenuItem>
										);
									})
								}
							</Select>
					...
					<Grid item xs={3} sm={3}>
						<FormControl fullWidth size="small">
							<TextField
								...
								fullWidth
								value={folder.code}
								onChange={ (e) => setFolder(folder => ({...folder, code: e.target.value})) }
								size="small"
								id="code"
								name="code"
								label="Code"
					...
					<Grid item xs={12} sm={12}>
						<FormControl fullWidth size="small">
							<TextField
								required
								fullWidth
								value={folder.designationAr}
								onChange={ (e) => setFolder(folder => ({...folder, designationAr: e.target.value})) }
								size="small"
								id="designationAr"
								name="designationAr"
					...
					<Grid item xs={12} sm={12}>
						<FormControl fullWidth size="small">
							<TextField
								required
								fullWidth
								value={folder.designationEn}
								onChange={ (e) => setFolder(folder => ({...folder, designationEn: e.target.value})) }
								size="small"
								id="designationEn"
								name="designationEn"
					...
					<Grid item xs={12} sm={12}>
						<FormControl fullWidth size="small">
							<TextField
								required
								fullWidth
								value={folder.designationFr}
								onChange={ (e) => setFolder(folder => ({...folder, designationFr: e.target.value})) }
								size="small"
								id="designationFr"
								name="designationFr"
